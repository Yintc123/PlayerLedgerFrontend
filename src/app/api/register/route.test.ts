...
function buildPost(body: object): NextRequest {
  return new NextRequest(new Request('http://localhost/api/register', {
    method: 'POST',
...
describe('POST /api/register (spec 02 §3.6)', () => {
  ...
  it('should strip browser-supplied client_id (cannot override BFF policy)', async () => {
    ...
    const req = buildPost({ username: 'newbie', password: 'StrongPass123!', client_id: 'public-web' })
    await POST(req)
  ...
  it('should pass through backend 409 username_taken', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ error: 'username_taken' }), { status: 409, headers: { 'content-type': 'application/json' } }),
    ))
