...
function buildReq(method: string, path: string, body?: string): { req: NextRequest, ctx: { params: Promise<{ path: string[] }> } } {
...
describe('BFF Proxy Handler - PUT / PATCH / DELETE', () => {
  ...
  it('should append last XFF segment as BFF immediate peer IP (spec §4.2 — not hardcoded 127.0.0.1)', async () => {
    ...
    const req = new NextRequest(new Request('http://localhost/api/items/42', {
      method: 'DELETE',
      headers: {
  ...
  it('should call OTel propagation.inject on outbound headers (spec 03 §4.6)', async () => {
    const injectSpy = vi.fn((_ctx, carrier: Headers, setter: { set: (c: Headers, k: string, v: string) => void }) => {
