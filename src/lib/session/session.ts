 ...
 * 在 Redis 存儲 session
 */
export async function storeSession(sid: string, session: SessionData): Promise<void> {
  ...
  try {
    await redis.setex(`session:${sid}`, ttlSeconds, JSON.stringify(session));
  } catch (err) {
    logger.error(
...
function revokeRefreshFamily(accessToken: string, refreshToken: string): void {
  ...
  fetch(url, {
    ...
    logger.warn(
      {
        type: 'auth.token.refresh.revoke_failed',
...
export async function getValidAccessToken(sid: string | undefined): Promise<string | null> {
  ...
  const lockSet = await redis.set(lockKey, '1', 'EX', config.session.refreshLockTtlSeconds, 'NX');
  ...
  if (lockSet) {
    ...
    try {
      ...
      const casResult = await redis.eval(
        ...
        Math.min(config.session.ttlSeconds, Math.floor((session.absoluteExpiresAt - now) / 1000))
      );
      ...
      if (err instanceof TokenRefreshError) {
        ...
        const outcome =
          err.backendError === 'replay_detected'
            ? 'replay_detected'
            : err.backendError === 'absolute_expired'
              ? 'absolute_expired'
              : 'expired';
...
async function refreshSessionTtl(sid: string, session: SessionData): Promise<void> {
  ...
  try {
    ...
    logger.warn(
      {
        type: 'session.ttl_refresh.error',
        error: err instanceof Error ? err.message : String(err),
      },
