...
export async function getDeepHealth(): Promise<HealthResponse> {
  const [redisCheck, apiCheck] = await Promise.allSettled([
    checkRedis(),
    checkApiServer(),
  ])

  const checks = {
    redis: redisCheck.status === 'fulfilled' ? redisCheck.value : { status: 'error' as const, error: 'Rejected', latencyMs: 0 },
    apiServer: apiCheck.status === 'fulfilled' ? apiCheck.value : { status: 'error' as const, error: 'Rejected', latencyMs: 0 },
  }
