...
/**
 * DropdownMenu — Radix 兜底的下拉選單（ADR 021 §強制要求 5）。
 * 外部點擊收回、Esc 收回、focus trap、portal、ARIA、鍵盤導航皆由 Radix 提供，
 * 不自寫事件監聽（對比 topups/MultiSelect 僅手動處理 Esc）。
 * 樣式沿用 design token（popover / accent / border / radius），不 hard-code 色票。
 */
...
function DropdownMenuTrigger({
  ...
  return (
    <DropdownMenuPrimitive.Trigger
      ...
      data-component="DropdownMenuTrigger"
      {...props}
    />
  );
}
...
function DropdownMenuGroup({ ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Group>) {
  return (
    <DropdownMenuPrimitive.Group
      ...
      data-component="DropdownMenuGroup"
      {...props}
    />
  );
}
