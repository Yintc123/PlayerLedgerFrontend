...
test.describe('Authentication Flow', () => {
  ...
  test('should set session cookie on successful login', async ({ context, page }) => {
    await page.goto('/api/login', { method: 'POST' })

    // Check if cookie is set (in real scenario)
    const cookies = await context.cookies()
    const sidCookie = cookies.find((c) => c.name === '__Host-sid' || c.name === 'sid')
  ...
  test('should clear session on logout', async ({ page }) => {
    await page.goto('/api/logout', { method: 'POST' })
