...
**註冊入口與成功 banner**（v1.x，配合 [`12-screen-register.md`](./12-screen-register.md)）：
...
- 完整版型、文案與測試清單見 [`12-screen-register.md §7`](./12-screen-register.md)；本節僅描述 `/login` 端的接點。
...
**範圍：** `POST /api/register` 經由 BFF 透傳至後端 `POST /auth/register`（OpenAPI 已定義）。本節描述 BFF 端 routing / 安全控制；對應的 register 頁面 UI 規格見 [`12-screen-register.md`](./12-screen-register.md)（v1.x 新增）。

> **歷史**：spec 第一版 v1 範圍只暴露 passthrough、不提供 UI，理由是給日後其他 client（手動 API、admin tool）使用。v1.x 補上 [`12`](./12-screen-register.md) 自助註冊頁，本節的 BFF 行為不變、只是多了一個前端消費者。
...
- **註冊成功不簽 token**（後端 OpenAPI 註記），caller 須另呼叫 `/api/login`；BFF 不主動串接，由 UI 決定 UX。[`12-screen-register.md §6.3`](./12-screen-register.md) 採「成功 → redirect `/login?registered=true` 由使用者手動登入」策略
...
const PUBLIC_PATHS = new Set<string>([
  ...
  '/api/login',
  '/api/logout',
  '/api/register',      // §3.6 — passthrough；UI 見 spec 12；CSRF + rate limit 仍適用
