...
### 3.6 註冊端點（passthrough，不對應 UI）

**範圍：** v1 暴露 `POST /api/register` 經由 BFF 透傳至後端 `POST /auth/register`（OpenAPI 已定義），但**不提供 register page、不提供前端 UI**，給日後其他 client（手動 API、admin tool、未來自助註冊頁面）使用。

**前端只做 routing / 安全控制，不寫表單。**
...
- **註冊成功不簽 token**（後端 OpenAPI 註記），caller 須另呼叫 `/api/login`；BFF 不主動串接，由 UI（未來才開發）決定 UX
