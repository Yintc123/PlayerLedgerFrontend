...
- **Browser → Upstream**：只轉發 `Content-Type` / `Accept` / `Accept-Language` / `Accept-Encoding`；BFF 自行注入 `Authorization` / `X-Request-ID` / `traceparent` / `tracestate` / `X-Forwarded-For`（append BFF 視角的 client IP）
- **Upstream → Browser**：只轉發 `Content-Type` / `Cache-Control` / `X-Request-ID` / `Retry-After`（後端 429 / 503 用以指示退避秒數，必須讓 Browser 看到）；丟棄所有 hop-by-hop（RFC 7230 §6.1 全集）與 `Set-Cookie`

> **`X-Forwarded-For` append 規則（RFC 7239 / 7230 §5.7.1）**：
> - BFF 從入站 request 讀取既有 XFF（CloudFront + API Gateway 已 append 兩跳）
> - **在尾端 append BFF 看到的 immediate peer IP**（即 API Gateway edge IP，由 `request.headers.get('x-forwarded-for')?.split(',').slice(-1)` 或 ECS task metadata 取得）
> - 整串連同新增的 BFF↔peer 連線資訊轉發到後端
> - **禁止直接 forward browser 自填的 XFF 而不 append BFF 自己這跳**——後端 Gin `c.ClientIP()` 仰賴 `SetTrustedProxies` 名單來決定走 XFF 哪個位置；BFF 必須加入後端 trusted proxies CIDR 並 append 自己的觀察值，否則後端會把 BFF peer（APIGW edge）當成真實 client IP，per-IP 限流變成「全部使用者同一 key」
> - 實作位置：`lib/api-client/headers.ts`
> - **後端對接事項**（cross-cutting，需後端 infrastructure.md §9.2 同步調整）：`cfg.Server.TrustedProxies` 必須含 ECS Task subnet CIDR + CloudFront edge ranges + API Gateway ranges，否則 BFF 的 XFF append 不被信任、`c.ClientIP()` 落回 BFF peer 反而失準。詳見 ADR 011 §「Client IP 提取」（請補 cross-ref 至後端設定要求）
...
- Timeout 觸發 → 回 **504**；upstream 5xx → 回 **502**；upstream 4xx → 透傳
- **客戶端斷線**（`req.signal` 觸發 abort）：socket 已被 runtime 關閉，Route Handler 無法再 write response body。處理方式為**僅 log / metric**，使用 tag `upstream.client_closed`（仿 nginx 499 語意但不作為 HTTP status）；handler 直接 throw 讓 fetch 中止，後續 response 由 Next.js runtime 自行處理
...
it('should append BFF immediate peer IP to incoming X-Forwarded-For before forwarding')   // ADR 011 + spec 01 §4.2 XFF append 規則
it('should generate X-Forwarded-For with single value when incoming request has none')
it('should NOT pass through other X-Forwarded-* headers (Host / Proto) untransformed')
...
it('should forward Content-Type / Cache-Control / X-Request-ID / Retry-After to browser')
it('should forward Retry-After header on upstream 429 to enable client backoff')
it('should forward Retry-After header on upstream 503 (service_unavailable)')
...
it('should log type=upstream.client_closed and abort upstream fetch when req.signal aborts (no response body)')
...
| `CSP_CONNECT_SRC_EXTRA` | ❌ | 空白分隔的 origin 清單，附加進 CSP `connect-src` 指令。v1 預設留空（僅 `'self'`）；新增 client-side telemetry / Sentry / RUM 時填入。詳見 §10.3 |
...
| Action 類別 | 範例 | 固定方式 |
|------------|------|---------|
| **具 `id-token: write` / `contents: write` / secrets 存取**（高權限） | `aws-actions/configure-aws-credentials`、`aws-actions/amazon-ecr-login`、`aws-actions/amazon-ecs-deploy-task-definition`、`docker/build-push-action`、`docker/setup-buildx-action`、`aquasecurity/trivy-action`、`github/codeql-action/*` | **必須 SHA-pin**（commit SHA，例 `@8c3f20df09ac63af7b3ae3d7c91f105f857d8497`），不接受 tag |
| 一般用途（無敏感權限） | `actions/checkout`、`actions/setup-node`、`actions/upload-artifact` | 接受 major tag（`@v4`），但 dependabot 會 PR 上 SHA pin |
| **特例：禁用** `@master` / `@main` / `@v*-rc` / `@latest` | — | 所有 actions 一律禁止這類可變指標 |
...
Content-Security-Policy:
  ...
  font-src 'self';
  connect-src 'self' ${CSP_CONNECT_SRC_EXTRA};   # ＊預留：client-side telemetry / CSP report endpoint
  report-uri /api/csp-report;                    # CSP 違規回報（spec 03 §6 補測試）
  report-to csp-endpoint;                        # 新版 reporting；report-uri 為 fallback
...
**為何 `connect-src` 留出 `CSP_CONNECT_SRC_EXTRA` 開關：** 未來導入 client-side telemetry（CloudWatch RUM、Sentry、外部 OTel collector）或 Web Vitals 回報時，必須允許瀏覽器 JS 連到那些 origin（[spec 03 §3 觀測性覆蓋缺口](./03-observability.md#3-metrics)）。v1 預設 `'self'`、env 留空；新增 endpoint 時設定 `CSP_CONNECT_SRC_EXTRA="https://rum.amazonaws.com https://sentry.io"`，避免日後改一條 hardcoded 字串就要重新 build。v1 也允許 `report-uri` / `report-to` 於本機 BFF endpoint 接收 CSP 違規（同 origin，不需額外開孔）。
...
- **ECR repository 必須 `imageTagMutability=IMMUTABLE`**：tag 一旦 push 就不可覆寫，杜絕「重 tag 同名指向新 image」攻擊 / 誤觸；deploy 永遠用具體 commit-based tag、禁止 `:latest` 入 ECR
- **掃描設定 `scanOnPush=true` + ECR enhanced scanning**：作為 CI 內 trivy 的雙保險（不同 vendor、不同節奏）
  ...
  deploy-staging:
    ...
    steps:
      ...
      - name: Smoke test
        ...
        env:
          EXPECTED_VERSION: ${{ needs.resolve-tag.outputs.tag }}
        run: |
          set -e
          ok=0
          for i in {1..10}; do
            BODY=$(curl -s -w '\n%{http_code}' https://staging.playerledger.com/api/health/deep) || true
            STATUS=$(echo "$BODY" | tail -n1)
            JSON=$(echo "$BODY" | sed '$d')
            if [ "$STATUS" = "200" ]; then
              DEPLOYED=$(echo "$JSON" | jq -r '.version // empty')
              if [ "$DEPLOYED" = "$EXPECTED_VERSION" ]; then
                echo "version OK: $DEPLOYED"
                ok=1; break
              fi
              echo "version mismatch: deployed=$DEPLOYED expected=$EXPECTED_VERSION (likely stale routing); retrying"
            fi
            sleep 5
          done
          [ "$ok" = "1" ] || { echo "Smoke test failed: /api/health/deep status=$STATUS or version mismatch"; exit 1; }
  ...
  "containerDefinitions": [
    {
      ...
      "stopTimeout": 60,
      "_comment_healthcheck": "刻意省略 container-level healthCheck：image 內已透過 Dockerfile HEALTHCHECK 跑 wget+grep（spec 04 §3.4），且 ECS 在 awsvpc 模式不會根據 container healthCheck 替換 task；真正決定路由可用的是 ALB Target Group health check（§9.4）。重複的 container healthCheck 只增加雜訊，且若寫成 curl 還會因 image 沒裝 curl 在 runtime 失敗",
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
